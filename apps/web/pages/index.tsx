...
import { COLOR_BASE_GRAY_MEDIUM, FONT_FAMILY_SYSTEM } from "design-tokens/dist/web/tokens";
import { useState } from "react";

const TokenDisplay = ({ token, onTokenUpdate }) => (
  <div style={{ display: "flex", alignItems: "center", fontFamily: FONT_FAMILY_SYSTEM }}>
    {token.name} - <input type="text" defaultValue={token.value} onBlur={(evt) => onTokenUpdate(token.name, evt.target.value)} />{" "}
    <div
      style={{
...
export default function Web() {
  ...
  async function sendTokenFile(tokenFile) {
    const res = await fetch('/api/tokens', { method: 'POST', body: JSON.stringify(tokenFile) })
  ...
  return (
    <>
      <div>
        <h1 style={{ color: COLOR_BASE_GRAY_MEDIUM, fontFamily: FONT_FAMILY_SYSTEM }}>Web Design Tokens</h1>
        {webTokenValues.map((token) => (
          <TokenDisplay token={token} onTokenUpdate={onUpdateToken('web')} />
        ))}
      </div>
      <div>
        <h1 style={{ color: COLOR_BASE_GRAY_MEDIUM, fontFamily: FONT_FAMILY_SYSTEM }}>Native Design Tokens</h1>
